...
//add local async method called register, give me code
async function register(req, res) {
    var user = null;
    try{
        const email = req.user.emails[0].value;
        //1 checkIfEmailAlreadyUsed
        var tempuser = await Users.findOne({ email: email });
        if (tempuser) {
            //throw new Error('Error: Account with this email-id already exists.');
            logger.info('[signupController][post:/signup] Account with this email-id already exists.');
            return;
        }
